...
/* ─────────────────────────────────────────────────────────
   TYPES DE SITES
   Chaque entrée = ce qu'un client reconnaît dans sa situation
───────────────────────────────────────────────────────── */
const SITE_TYPES = [
  {
    id: 'vitrine',
    ...
    svg: (
      <svg viewBox="0 0 480 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%' }}>
        ...
        {/* 3 services cards */}
        {[0,1,2].map(i => (
          <rect key={i} x={40 + i*142} y="220" width="128" height="60" rx="8" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.08)"/>
        ))}
  ...
  },
  {
    ...
    svg: (
      <svg viewBox="0 0 480 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%' }}>
        ...
        {[56,80,104,128,152].map((y,i) => (
          <rect key={i} x="12" y={y} width="72" height="14" rx="3" fill={i===0?"rgba(232,160,32,0.6)":"rgba(255,255,255,0.1)"}/>
        ))}
        ...
        {[160,178,196,214,232,250].map((y,i) => (
          <rect key={i} x="388" y={y} width="68" height="8" rx="3" fill={i===0?"rgba(255,255,255,0.25)":"rgba(255,255,255,0.09)"}/>
        ))}
      </svg>
    ),
  },
  {
    ...
    inclus: ['Calendrier de disponibilités', 'Formulaire de réservation', 'Confirmations automatiques', 'Tableau de bord de gestion', 'Notifications email / SMS'],
    from: '520 000 FDJ',
    ...
    svg: (
      <svg viewBox="0 0 480 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%' }}>
        ...
        {/* Réservation card */}
        <rect x="272" y="64" width="184" height="204" rx="10" fill="#fff" opacity="0.06" stroke="rgba(232,160,32,0.4)"/>
        <rect x="284" y="78" width="120" height="12" rx="3" fill="rgba(255,255,255,0.5)"/>
        {/* Calendrier mini */}
        <rect x="284" y="98" width="160" height="96" rx="6" fill="rgba(255,255,255,0.04)"/>
        {[0,1,2,3,4,5,6].map(i => (
          <rect key={i} x={284+i*23} y={102} width={18} height={10} rx="2" fill="rgba(255,255,255,0.15)"/>
        ))}
        {[0,1,2,3,4].map(row => [0,1,2,3,4,5,6].map(col => {
          const active = (row===1&&col===2)||(row===2&&col===4);
          const today  = row===0&&col===3;
          return (
            <rect key={`${row}-${col}`} x={284+col*23} y={118+row*16} width={18} height={12} rx="3"
              fill={active?"#E8A020":today?"rgba(29,158,117,0.4)":"rgba(255,255,255,0.05)"}/>
          );
        }))}
        ...
        {/* Galerie bas */}
        {[0,1,2].map(i => (
          <rect key={i} x={i*88} y="220" width="80" height="60" rx="6" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.07)"/>
        ))}
      </svg>
    ),
  },
  {
    ...
    inclus: ['2 à 3 langues simultanées (FR / EN / AR)', 'Interface admin unifiée', 'URLs localisées', 'SEO multilingue', 'Sélecteur de langue discret'],
    from: 'Sur devis',
    ...
    svg: (
      <svg viewBox="0 0 480 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%' }}>
        ...
        <rect x="376" y="18" width="8" height="16" rx="1" fill="rgba(255,255,255,0.3)"/>
        <rect x="400" y="14" width="60" height="24" rx="5" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.1)"/>
        <rect x="408" y="20" width="44" height="12" rx="2" fill="rgba(255,255,255,0.15)"/>
        {/* Globe */}
        <circle cx="240" cy="168" r="90" fill="none" stroke="rgba(29,158,117,0.2)" strokeWidth="1.5"/>
        <circle cx="240" cy="168" r="60" fill="none" stroke="rgba(232,160,32,0.15)" strokeWidth="1"/>
        <ellipse cx="240" cy="168" rx="36" ry="90" fill="none" stroke="rgba(29,158,117,0.15)" strokeWidth="1" strokeDasharray="4 3"/>
        <ellipse cx="240" cy="168" rx="90" ry="28" fill="none" stroke="rgba(29,158,119,0.15)" strokeWidth="1" strokeDasharray="4 3"/>
        <circle cx="240" cy="168" r="14" fill="rgba(232,160,32,0.3)"/>
        {/* 3 langues */}
        {[
          { cx:160, cy:120, label:'FR', color:'rgba(29,158,117,0.6)' },
          { cx:320, cy:120, label:'EN', color:'rgba(232,160,32,0.6)' },
          { cx:240, cy:240, label:'AR', color:'rgba(159,225,203,0.5)' },
        ].map(l => (
          <g key={l.label}>
            ...
            <line x1={l.cx} y1={l.label==='AR'?l.cy-22:l.cy+22} x2="240" y2={l.label==='AR'?228:154} stroke="rgba(255,255,255,0.1)" strokeDasharray="3 3"/>
          </g>
        ))}
      </svg>
    ),
  },
  {
    ...
    desc: 'Pour les entreprises qui veulent présenter leurs réalisations, produits ou projets de manière visuelle et professionnelle sans vendre en ligne.',
    pourQui: ['Agences', 'Architectes & designers', 'Photographes', 'Constructeurs & artisans'],
    ...
    svg: (
      <svg viewBox="0 0 480 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%' }}>
        ...
        {['Tous','Architecture','Intérieur','Commercial','Résidentiel'].map((label, i) => (
          <rect key={i} x={32+i*86} y={67} width={80} height={18} rx="4"
            fill={i===0?"rgba(232,160,32,0.6)":"rgba(255,255,255,0.05)"}/>
        ))}
    ...
    <>
      ...
      <div style={{ paddingTop: '32px', position: 'relative', zIndex: 2 }}>
        <div className="container-custom" style={{ paddingBottom: '64px' }}>
          ...
          {/* Fil d'Ariane */}
          <nav style={{ display: 'flex', alignItems: 'center', gap: '8px', fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--ink)', marginBottom: '40px', letterSpacing: '0.04em' }}>
            <BreadcrumbLink href="/">Accueil</BreadcrumbLink>
            <span style={{ opacity: 0.4 }}>/</span>
            <BreadcrumbLink href="/services">Services</BreadcrumbLink>
            <span style={{ opacity: 0.4 }}>/</span>
            <span style={{ color: 'var(--or-dark)' }}>Développement web</span>
          </nav>
          ...
          <div style={{ maxWidth: '760px' }}>
            ...
            <p className="section-sub rev d2" style={{ fontSize: '18px', marginBottom: '0' }}>
                ...
                <div style={{
                  ...
                  <div style={{ order: idx % 2 === 0 ? 0 : 1 }}>
                    ...
                    <div style={{
                      ...
                      <div>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: '20px', color: 'var(--ink)', marginBottom: '2px' }}>
                          {type.from === 'Sur devis' ? '' : 'À partir de'}
                        </div>
                        <div style={{
                      ...
                      <div>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: '20px', color: 'var(--ink)', marginBottom: '2px' }}>Durée</div>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>
                          {type.duration}
                        </div>
                      </div>
                      <Link
      ...
      <div style={{
        ...
        <div className="container-custom">
          <p style={{
            ...
            Vous ne vous reconnaissez dans aucun de ces profils ?<br/>
